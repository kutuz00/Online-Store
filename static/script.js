...
const app = new Vue({
    ...
    methods: {
        ...
        addTocart(e) {
            ...
            const item = this.items.find((item) => item.id_product === e.target.id);
    ...
    mounted() {
        fetch('http://localhost:3000/catalog', { 'Content-Type': 'application/json; charset=UTF-8' })
            .then(responce => responce.json())
            .then((responce) => this.items = responce)
            .then((responce) => this.filteredItems = responce)
            .catch(error => this.noData = !this.noData);
        fetch('http://localhost:3000/cart', { 'Content-Type': 'application/json; charset=UTF-8' })
